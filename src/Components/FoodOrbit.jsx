...
function CousinMoon({ cousin, mood, radius, duration, startAngle, size, reducedMotion }) {
  const moonBody = (
    <div
...
export default function DishCluster({
  ...
  const orbitRadius = (i) => mainSize * 0.5 + i * (mainSize * 0.15);
  ...
  if (reducedMotion) {
    return (
      <div style={{ position: "absolute", left: x, top: y, width: 0, height: 0, zIndex }}>
        {cluster}
      </div>
    );
  }
